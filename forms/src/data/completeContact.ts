...
export async function completeContact(previousState: ActionState, formData: FormData) {
  // emphasize the duration of the server call
  await new Promise((resolve) => setTimeout(resolve, 1000));
  const parsedResult = completeSchema.safeParse(Object.fromEntries(formData));
